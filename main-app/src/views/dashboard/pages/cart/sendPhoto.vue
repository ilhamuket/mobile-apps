            ...
            <v-row>
              ...
              >
                <span
                  v-if="detail.class"
                  class="font-spartan"
                >Name : {{ detail.class.name }} <br>
                  levels : {{ detail.class.levels }}</span>
              </v-col>
              <v-divider />
              <v-col
                class="d-flex justify-center"
...
<script>
  export default {
    ...
    computed: {
      detail () {
        return this.$store.state.cart.detail
      },
    },
    mounted () {
      this.getDetailCartEnsikloLive()
    },
    methods: {
      onFileChange (event) {
        ...
        if (this.files.size > 2000000) {
          ...
          this.$store
            .dispatch("payment/sendProofPayment", {
              files: this.files,
      ...
      getDetailCartEnsikloLive () {
        this.$store.dispatch("cart/getDetailCartEnsikloLive", {
          id: this.$route.params.id,
          entities: "bank,user,class",
        })
      },
      onClick () {
