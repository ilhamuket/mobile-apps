...
class InstructorHasClassesController extends Controller
{
    /**
    ...
    {
        try {
            $master = StudioClass::whereHas('instructor_v2', function (Builder $query) use ($id) {
                $query->where('instructor_id', $id);
            })->entities($request->entites)
